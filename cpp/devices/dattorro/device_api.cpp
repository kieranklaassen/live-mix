// Flat C ABI (cpp/common/device_api.h) over the Dattorro device for the
...
namespace {
livemix::DattorroDevice g_device;
}
...
void device_set_param(int param_id, float value) {
  g_device.set_param(static_cast<livemix::DattorroParam>(param_id), value);
}
...
int device_max_block_frames(void) {
  return livemix::DattorroDevice::kMaxBlockFrames;
}
