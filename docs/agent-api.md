...
Every mixer capability is a tool a model can call (plan KD4 / KTD9): the 46
...
| `consent`                        | `[]`                | Structural operations (`track_*`, `group_*`, `return_*`, `device_add/remove/move`, `send_add/remove`, `lane_add/remove`, `modulator_*`, `route_add/remove`, `strip_rename/route`, `score_rename`) need `'structure'`; arrangement edits (`clip_*`, `source_*`, `transport_loop`, breakpoints) need `'arrange'`; `batch` needs every child's. Parameter-level operations and all intents need none. `grantConsent` / `revokeConsent` at runtime. |
